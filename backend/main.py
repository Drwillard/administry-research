...
from analytics import ANON_SALT, run_pipeline, load_sentiment_trends
...
def author_burnout(
    ...
    """
    try:
        results = load_sentiment_trends("notes_long_by_author.parquet", "author_key", min_notes, slope_threshold)
...
def clients_at_risk(
    ...
    """
    try:
        results = load_sentiment_trends("notes_long_by_client.parquet", "client_key", min_notes, slope_threshold)
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail="No data found. Run /ingest first.")
    except ValueError as e:
        raise HTTPException(status_code=422, detail=str(e) + ". Re-run /ingest.")
